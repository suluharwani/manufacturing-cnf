...
$('.tambahMaterial').on('click', function() {
  $.when(
    ...
    $.ajax({
      url: base_url + '/material/get_last_material_id', // Endpoint baru untuk mendapatkan ID terakhir
      method: 'GET',
      dataType: 'json'
    })
  ).done(function(typesResponse, satuanUkuranResponse, lastIdResponse) {
    // Extract data
    const typesData = typesResponse[0];
    const satuanUkuranData = satuanUkuranResponse[0];
    const lastMaterialId = lastIdResponse[0] ? lastIdResponse[0].last_id : 0;

    if (Array.isArray(typesData) && Array.isArray(satuanUkuranData)) {
      let typeOptions = typesData.map(type => `<option value="${type.id}" data-code="${type.kode}">${type.nama}</option>`).join('');
      let satuanUkuranOptions = satuanUkuranData.map(satuan => `<option value="${satuan.id}">${satuan.nama}</option>`).join('');

      // Generate kode otomatis berdasarkan type pertama yang dipilih dan status KITE
      const firstTypeCode = typesData[0] ? typesData[0].kode : 'DEF';
      const newMaterialId = parseInt(lastMaterialId) + 1;
      const kiteStatus = 'KITE'; // Default value
      const kiteSuffix = kiteStatus === 'KITE' ? 'KT' : 'NK';
      const autoGeneratedCode = `${firstTypeCode}${newMaterialId.toString().padStart(3, '0')}${kiteSuffix}`;
      ...
      Swal.fire({
        ...
        <form id="form_add_data">
          <div class="form-group">
            <label for="kode">Kode</label>
            <input type="text" class="form-control" id="kode" value="${autoGeneratedCode}" placeholder="Kode" readonly>
            <small class="form-text text-muted">Kode akan digenerate otomatis berdasarkan jenis material dan status KITE</small>
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            <label for="kite">Import Export Status</label>
      ...
      // Event listener untuk perubahan type dan status KITE
      $(document).on('change', '#type, #kite', function() {
        const selectedType = $('#type').find('option:selected');
        const typeCode = selectedType.data('code');
        const kiteStatus = $('#kite').val();
        const newMaterialId = parseInt(lastMaterialId) + 1;
        const kiteSuffix = kiteStatus === 'KITE' ? 'KT' : 'NK';
        const newAutoGeneratedCode = `${typeCode}${newMaterialId.toString().padStart(3, '0')}${kiteSuffix}`;
        
        $('#kode').val(newAutoGeneratedCode);
      });
    } else {
      Swal.fire({
        icon: 'error',
    ...
    $.ajax({
      ...
      const satuanUkuranData = satuanUkuranResponse[0];
          ...
          if (Array.isArray(typesData) && Array.isArray(satuanUkuranData) && materialData) {
            let typeOptions = typesData.map(type => `<option value="${type.id}" ${type.id == materialData.type_id ? 'selected' : ''}>${type.kode} - ${type.nama}</option>`).join('');
