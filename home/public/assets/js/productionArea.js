...
$('#isiGudangProduksi').on('click','.editData',function(){
  let id = $(this).attr('id');
  ...
  Swal.fire({
    ...
    $.ajax({
      type : "POST",
      url  : base_url+'/ProductionController/update',
      async : false,
      // dataType : "JSON",
      data : {params},
...
$.ajax({
    type : "POST",
    url  : base_url+"ProductionController/deletedData",
    async : false,
    dataType : 'json',
    data:{},
    success: function(data){
...
$('#tabelRestore').on('click','.purgeData',function(){
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      $.ajax({
        type : "POST",
        url  : base_url+"ProductionController/purgeData",
        async : false,
        data:{param:param},
        success: function(data){
         tableRestoreData()
...
$('#tabelRestore').on('click','.restoreData',function(){
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.ajax({
        type  : 'post',
        url   : base_url+'/ProductionController/restoreData',
        async : false,
        // dataType : 'json',
        data:{param:param},
        success : function(data){
...
$('.tambahWarehouse').on('click',function(){
    ...
    Swal.fire({
      ...
      $.ajax({
        type : "POST",
        url  : base_url+'/ProductionController/create',
        async : false,
        // dataType : "JSON",
        ...
        success: function(data){
          ...
          Swal.fire({
            position: 'center',
            icon: 'success',
            title: `Production Area berhasil ditambahkan.`,
            showConfirmButton: false,
            timer: 1500
          })
        },
  ...
  function dataGudang(){
    $.ajax({
      type : "POST",
      url  : base_url+"ProductionController/gudang_list",
      async : false,
      success: function(data){
       tableGudang(data);
  ...
  function tableGudang(data){
   ...
   $('#isiGudangProduksi').html(table)
  }
$('#isiGudangProduksi').on('click','.delete',function(){
  id = $(this).attr('id');
  nama = $(this).attr('nama');
  Swal.fire({
    title: 'Apakah anda yakin?',
    ...
    if (result.isConfirmed) {
      ...
      $.ajax({
        type : "POST",
        url  : base_url+"ProductionController/delete",
        async : false,
        data:{param:param},
        success: function(data){
          dataGudang()
