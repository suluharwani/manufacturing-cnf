...
$('#isiDepartment').on('click','.editData',function(){
  let id = $(this).attr('id');
...
$('.tambahWarehouse').on('click',function(){
  ...
  function dataGudang(){
    $.ajax({
      type : "POST",
      url  : base_url+"department/department_list",
      async : false,
      success: function(data){
       tableDepartment(data);
  ...
  function tableDepartment(data){
   ...
   $('#isiDepartment').html(table)
  }
$('#isiDepartment').on('click','.delete',function(){
  id = $(this).attr('id');
  nama = $(this).attr('nama');
  Swal.fire({
    title: 'Apakah anda yakin?',
