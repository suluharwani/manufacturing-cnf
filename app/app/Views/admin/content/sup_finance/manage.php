...
<div class="container-fluid pt-4 px-4">
        ...
        <div class="col-md-12">
            <div class="bg-light rounded p-4">
                <div class="d-flex align-items-center justify-content-between mb-4">
                    <h4 class="mb-0">Supplier Finance: <?= $supplier['supplier_name'] ?></h4>
    ...
    <small class="text-muted">Last updated: <?= date('d M Y H:i', strtotime($account['updated_at'] ?? 'now')) ?></small>
                ...
                <ul class="nav nav-tabs mb-4" id="financeTabs" role="tablist">
                    ...
                    <li class="nav-item" role="presentation">
                        <button class="nav-link" id="po-tab" data-bs-toggle="tab" data-bs-target="#po" type="button" role="tab">PO History</button>
                    </li>
                </ul>
                            ...
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><?= $purchase['tanggal_nota'] ?></td>
                                        <td><?= number_format($purchase['total'] ?? 0, 2, ',', '.') ?></td>
                                        <td><?= $purchase['status_pembayaran'] == 0 ? 'Unpaid' : 'Paid' ?></td>
                            ...
                            <table class="table table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Date</th>
                                        <th>Total</th>
                                        <th>Arrival Target</th>
                                        <th>Remarks</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php foreach ($poHistory as $po): ?>
                                    <tr>
                                        <td><?= $po['code'] ?></td>
                                        <td><?= $po['date'] ?></td>
                                        <td><?= number_format($po['total'] ?? 0, 2, ',', '.') ?></td>
                                        <td><?= $po['arrival_target'] ?></td>
...
<!-- Transaction Modal (same as in accounts.php) -->
<!-- Transaction Modal -->
...
<!-- Transaction Details Modal (same as in accounts.php) -->
<div class="modal fade" id="transactionDetailsModal" tabindex="-1" aria-labelledby="transactionDetailsModalLabel" aria-hidden="true">
    <!-- Modal content from accounts.php -->
</div>
...
<script>
    ...
    function openTransactionModal(accountId) {
        ...
        $('#transaction_date').val(new Date().toISOString().split('T')[0]); // Set tanggal hari ini
...
function submitTransaction() {
    ...
    $.ajax({
        url: '<?= base_url('supfinance/addtransaction') ?>',
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
    ...
    function viewTransactionDetails(transactionId) {
        ...
        $.get('<?= base_url('supfinance/gettransactiondetails') ?>/' + transactionId, function(response) {
            if (response.status === 'success') {
                ...
                if (transaction.documents && transaction.documents.length > 0) {
                    ...
                    transaction.documents.forEach(doc => {
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            ...
                            <a href="<?= base_url('supfinance/downloaddocument') ?>/${doc.id}" class="btn btn-sm btn-outline-primary" target="_blank">
...
document.getElementById('confirmDeleteBtn').addEventListener('click', function() {
    ...
    $.ajax({
        url: '<?= base_url('supfinance/deletetransaction') ?>/' + transactionToDelete,
        type: 'DELETE',
        dataType: 'json',
        beforeSend: function() {
