...
<body>
    ...
    <?php
    // Calculate total cost from Query 2 (Finishing)
    $totalFinishingCost = 0;
    foreach ($query2 as $row) {
        $penggunaan = floatval($row['penggunaan']);
        $last_price = floatval($row['last_price']);
        $rate = floatval($row['rate']);
        if ($rate != 0) {
            $totalFinishingCost += ($last_price * $penggunaan) / $rate;
        }
    }
    
    // Calculate total cost from Query 3 (Modul)
    $totalModulCost = 0;
    foreach ($query3 as $row) {
        ...
        $rate = floatval($row['rate']);
        if ($rate != 0) {
            $totalModulCost += ($last_price * $penggunaan) / $rate;
        }
    }
    
    $totalCost = $totalFinishingCost + $totalModulCost;
    ?>
        ...
        <tbody>
            <?php foreach ($query1 as $row): ?>
                <tr>
                    ...
                    <td><?= $row['finishing'] ?></td>
                    <td><?= date("F j, Y") ?></td>
                    <td class="total-cost">Rp <?= number_format($totalCost, 0, ',', '.') ?></td>
                </tr>
            <?php endforeach; ?>
        </tbody>
    </table>

    <!-- Rest of your existing code for Query 2 and Query 3 tables remains the same -->
