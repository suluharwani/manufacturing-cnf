...
<script>
    $(document).ready(function() {
        ...
        var table = $('#tabel_serverside').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url": "<?= base_url('supfinance/getAccounts') ?>",
                "type": "POST"
            },
            // Di bagian columns DataTables:
"columns": [
    ...
    },
    {
        ...
        "className": "text-center",
        "render": function(data, type, row) {
            return '<a href="<?= base_url('supfinance/manage') ?>/' + data + '" class="btn btn-sm btn-primary">Manage</a>';
        }
    }
]
        });
            ...
            if (response.status === 'success') {
                ...
                if (transaction.documents && transaction.documents.length > 0) {
                    ...
                    transaction.documents.forEach(doc => {
                        html += `<li>
                            <a href="<?= base_url('supfinance/downloaddocument') ?>/${doc.id}" target="_blank">${doc.file_name}</a>
                            (${doc.document_type})
                        </li>`;
                    });
                    html += '</ul></div>';
