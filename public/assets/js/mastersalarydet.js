...
$(document).ready(function () {
  ...
  function loadPayrollData() {
    $.ajax({
      url:
        base_url +
        "/MasterPenggajianDetailController/dataEmployeeMaster/" +
        masterId,
      type: "GET",
      dataType: "json",
      success: function (response) {
        ...
        response.forEach(function (item) {
          var row = `<tr>
                            ...
                            <td>${formatDate(
                              item.tanggal_akhir_penggajian
                            )}</td>
                           
                            <td><button class="btn btn-success btn-sm showPresensi" idKaryawan = "${
                              item.karyawan_id
                            }" pinKaryawan="${
            item.pegawai_pin
          }" tglAwal = "${formatDate(
            item.tanggal_awal_penggajian
          )}" tglAkhir= "${formatDate(
            item.tanggal_akhir_penggajian
          )}" data-id="${item.karyawan_id}" nama="${
            item.pegawai_nama
          }">Attendance</button>
                                <a href="javascript:void(0);" class="btn btn-primary btn-sm varTunjangan"  name = "${
                                  item.pegawai_nama
                                }" id="${item.karyawan_id}" pin="${
            item.pegawai_pin
          }">Tunjangan</a>
                                <a href="javascript:void(0);" class="btn btn-warning btn-sm varPotongan" name = "${
                                  item.pegawai_nama
                                }" id="${item.karyawan_id}" pin="${
            item.pegawai_pin
          }">Potongan</a>
                                <button class="btn btn-default btn-sm print-slip" data-id="${
                                  item.karyawan_id
                                }">Print Slip</button>
                                <button class="btn btn-default btn-sm print-slip-v2" data-id="${
                                  item.karyawan_id
                                }">Print Slip V2</button>
                                <button class="btn btn-default btn-sm print-presensi" namaKaryawan = "${
                                  item.pegawai_nama
                                }"  idKaryawan = "${
            item.karyawan_id
          }" pinKaryawan="${item.pegawai_pin}" tglAwal = "${formatDate(
            item.tanggal_awal_penggajian
          )}" tglAkhir= "${formatDate(
            item.tanggal_akhir_penggajian
          )}" data-id="${item.karyawan_id}" nama="${
            item.pegawai_nama
                                ...
                                <button class="btn btn-danger btn-sm delete-from-payroll" data-id="${
                                  item.karyawan_id
                                }">Delete</button> </td>
                        </tr>`;
      ...
      success: function (response) {
        ...
        $.ajax({
          ...
          success: function (workDays) {
            ...
            let html = generateAttendanceTable(
              groupedData,
              workDays,
              id,
              startDate,
  ...
  function generateAttendanceTable(
    ...
    $.each(groupedData, function (date, entries) {
      const { inTime: originalInTime, outTime: originalOutTime } =
        getInOutTimes(entries);
      ...
      if (inTime && outTime) {
        ...
        let {
          ...
          overtimeMinutes2,
          overtimeMinutes3,
        } = calculateWorkMinutes(inTime, outTime, workDaySetting, date);
        ...
        const overtime2Display = `${Math.floor(overtimeMinutes2 / 60)} jam ${
          Math.floor(overtimeMinutes2) % 60
        } menit (<span style="color: green;">Diakui: ${Math.floor(
          roundedOvertime2 / 60
        )} jam ${roundedOvertime2 % 60} menit</span>)`;
        const overtime3Display = `${Math.floor(overtimeMinutes3 / 60)} jam ${
          Math.floor(overtimeMinutes3) % 60
        } menit (<span style="color: green;">Diakui: ${Math.floor(
          roundedOvertime3 / 60
        )} jam ${roundedOvertime3 % 60} menit</span>)`;

        // Apply "Tidak Valid" label if overtime1 is less than 75 minutes and greater than 0
        const overtime1Display =
          overtimeMinutes1 === 0 && overtimeMinutes1 < 75
            ? `<span style="color: red;">(${Math.floor(
                overtimeMinutes1 / 60
              )} jam ${Math.floor(overtimeMinutes1 % 60)})Tidak Valid</span>`
            : `${Math.floor(overtimeMinutes1 / 60)} jam ${Math.floor(
                overtimeMinutes1 % 60
              )} menit`;
        ...
        html += `
            ...
            <td>${Math.floor(normalWorkMinutes / 60)} jam ${Math.floor(
          normalWorkMinutes % 60
        )} menit</td>
            ...
            id="${id}" startDate="${startDate}" endDate="${endDate}" pin="${
          entries[0].pin
        }" date="${date}"><i class="fas fa-plus"></i>
            Tambah
            </a>
    ...
    <tr>
    <td>Periode</td>
    <td>${formatDateWithDayIndonesian(
      startDate
    )} - ${formatDateWithDayIndonesian(endDate)}</td>
    </tr>
    <tr>
    <td>Hari Kerja</td>
    ...
    <td>${Math.floor(totalDurationMinutes / 60)} jam ${Math.floor(
    ...
    <td>${Math.floor(totalOvertime1Minutes / 60)} jam ${Math.floor(
      totalOvertime1Minutes % 60
    )} menit</td>
    </tr>
    <tr>
    <td>Total Lembur 2</td>
    <td>${Math.floor(totalOvertime2Minutes / 60)} jam ${Math.floor(
      totalOvertime2Minutes % 60
    )} menit</td>
    </tr>
    <tr>
    <td>Total Lembur 3</td>
    <td>${Math.floor(totalOvertime3Minutes / 60)} jam ${Math.floor(
      totalOvertime3Minutes % 60
    )} menit</td>
    </tr>
    </tbody>
    </table>`;
    ...
    Swal.fire({
      ...
      preConfirm: () => {
        const attendanceDate =
          Swal.getPopup().querySelector("#attendanceDate").value;
  ...
  function calculateWorkMinutes(inTime, outTime, workDaySetting, date) {
    ...
    if (workDaySetting) {
      ...
      const overtimeStart1 = new Date(
        `${date} ${workDaySetting.overtime_start_1}`
      );
      const overtimeEnd1 = new Date(`${date} ${workDaySetting.overtime_end_1}`);
      const overtimeStart2 = new Date(
        `${date} ${workDaySetting.overtime_start_2}`
      );
      const overtimeEnd2 = new Date(`${date} ${workDaySetting.overtime_end_2}`);
      const overtimeStart3 = new Date(
        `${date} ${workDaySetting.overtime_start_3}`
      );
      ...
      if (inTime < workBreakEnd && outTime > workBreakEnd) {
        const breakStartTime =
          inTime < workBreakStart ? workBreakStart : inTime; // Use the later time
      ...
      if (outTime > overtimeStart1) {
        overtimeMinutes1 =
          (Math.min(outTime, overtimeEnd1) - overtimeStart1) / (1000 * 60); // Convert to minutes
      }
      if (outTime > overtimeStart2) {
        overtimeMinutes2 =
          (Math.min(outTime, overtimeEnd2) - overtimeStart2) / (1000 * 60); // Convert to minutes
      }
      if (outTime > overtimeStart3) {
        overtimeMinutes3 =
          (Math.min(outTime, overtimeEnd3) - overtimeStart3) / (1000 * 60); // Convert to minutes
      }
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        $.ajax({
          ...
          url:
            base_url +
            "/MasterPenggajianDetailController/deleteEmployeeFromPayroll",
          data: { employeeId: employeeId, masterId: masterId },
          success: function (response) {
            loadPayrollData();
            Swal.fire({
  ...
  function generateSalarySlipHTML(employeeData) {
    ...
    $.each(employeeData.deduction, function (index, deduction) {
      deductionList += `<tr><td>${deduction.Nama}</td><td>${numberFormat(
        deduction.amount
      )}</td></tr>`;
    });
    ...
    $.each(employeeData.allowance, function (index, allowance) {
      allowanceList += `<tr><td>${allowance.Nama}</td><td>${numberFormat(
        allowance.amount
      )}</td></tr>`;
    });
    ...
    const slipHTML = `
        <div class="salary-slip">
            ...
            <p><strong>Rekening:</strong> ${pegawai.bank} - ${
      pegawai.bank_account
    }</p>
            <p><strong>Periode Gaji:</strong> ${formatDate(
              employeeData.tanggal_awal_penggajian
            )} - ${formatDate(employeeData.tanggal_akhir_penggajian)}</p>
            
            <h3>Rincian Gaji</h3>
            <table class="main-table">
                <tr><td><strong>Nama</strong></td><td><strong>Amount (Rp)</strong></td></tr>
                <tr><td>Gaji Harian Senin-Jumat</td><td>${
                  employeeData.salary_slip_details.basic_salary
                }</td></tr>
                <tr><td>Gaji Lembur 16.45-18.00</td><td>${
                  employeeData.salary_slip_details.overtime1_salary
                }</td></tr>
                <tr><td>Gaji Lembur 18.30-20.00</td><td>${
                  employeeData.salary_slip_details.overtime2_salary
                }</td></tr>
                <tr><td>Gaji Lembur >20.30</td><td>${
                  employeeData.salary_slip_details.overtime3_salary
                }</td></tr>
                <tr><td>Gaji Sabtu</td><td>${
                  employeeData.salary_slip_details.saturday_salary
                }</td></tr>
                <tr><td>Gaji Minggu</td><td>${
                  employeeData.salary_slip_details.sunday_salary
                }</td></tr>
                <tr><td>Gaji Kotor</td><td>${
                  employeeData.salary_slip_details.gross_salary
                }</td></tr>
                <tr><td>Tunjangan</td><td>${
                  employeeData.salary_slip_details.allowances
                }</td></tr>
                <tr><td>Potongan</td><td>${
                  employeeData.salary_slip_details.deductions
                }</td></tr>
                <tr><td><strong>Gaji Bersih</strong></td><td><strong>${
                  employeeData.salary_slip_details.net_salary
                }</strong></td></tr>
            </table>
            <hr>
  ...
  $(document).on("click", ".print-slip", function () {
    ...
    $.ajax({
      url:
        base_url +
        "/MasterPenggajianDetailController/getEmployeeSalarySlip/" +
        employeeId +
        "/" +
        masterId,
      type: "GET",
  ...
  function generateSalarySlipPDF(employeeData) {
    ...
    // Header slip gaji
    addText("Slip Gaji Karyawan", 105, 20, {
      size: 16,
      style: "bold",
      align: "center",
    });
    ...
    addText(
      `Periode Gaji: ${formatDate(
        employeeData.tanggal_awal_penggajian
      )} - ${formatDate(employeeData.tanggal_akhir_penggajian)}`,
      15,
      70
    );
    ...
    const salaryData = [
      ["Nama", "Amount (Rp)"],
      [
        "Gaji Harian Senin-Sabtu",
        employeeData.salary_slip_details.salary_mon_sat,
      ],
    ...
    currentY += 15;
    addText(
      `Slip gaji ini dihasilkan pada ${formatDate(new Date())}`,
      15,
      currentY,
      { size: 10 }
    );
    ...
    // Simpan PDF
    doc.save(
      `Slip_Gaji_${employeeData.pegawai_nama}_${employeeData.karyawan_id}.pdf`
    );
  }
      ...
      success: function (data) {
        ...
        if (data.length > 0) {
          data.forEach(function (allowance, index) {
            ...
            <td>
            <button class="btn btn-danger deleteAllowanceBtn" data-id="${
              allowance.id
            }">Delete</button>
            </td>
            </tr>
      ...
      success: function (data) {
        ...
        if (data.length > 0) {
          data.forEach(function (deduction, index) {
            ...
            <td>
            <button class="btn btn-danger deleteDeductionBtn" data-id="${
              deduction.id
            }">Delete</button>
            </td>
            </tr>
  ...
  function generateRekapGajiHTML(employeeDataList) {
    ...
    employeeDataList.forEach((employeeData, index) => {
      ...
      rekapHTML += `
            <div class="salary-slip ${index > 0 ? "page-break" : ""}">
                ...
                <h4>Rincian Gaji</h4>
                <table>
                    <tr><td>Gaji Pokok</td><td>${formatRupiah(
                      employeeData.total_salary
                    )}</td></tr>
                    <tr><td>Total Jam Kerja</td><td>${
                      employeeData.total_work_Hours
                    }</td></tr>
                    <tr><td>Lembur 1</td><td>${
                      employeeData.total_overtime1_Hours
                    }</td></tr>
                    <tr><td>Lembur 2</td><td>${
                      employeeData.total_overtime2_Hours
                    }</td></tr>
                    <tr><td>Lembur 3</td><td>${
                      employeeData.total_overtime3_Hours
                    }</td></tr>
                    <tr><td>Kerja Hari Minggu</td><td>${
                      employeeData.sunday_work_Hours
                    }</td></tr>
                </table>
                <hr>
            </div>`;
    ...
    rekapHTML += `
        <div class="total-gaji">
            ...
            <p><strong>Total Gaji Semua Karyawan:</strong> ${formatRupiah(
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        $.ajax({
          ...
          success: function (workDays) {
            const groupedData = groupAttendanceData(data);
            const attendanceHTML = generatePrintableAttendanceTable(
              groupedData,
              workDays,
              startDate,
              endDate,
  ...
  function generatePrintableAttendanceTable(
    groupedData,
    workDays,
    startDate,
    endDate,
    ...
    <p>Periode: ${formatDateWithDayIndonesian(
      startDate
    )} - ${formatDateWithDayIndonesian(endDate)}</p>
      ...
      if (inTime && outTime) {
        ...
        html += `
          ...
          normalWorkMinutes % 60
                ...
                <td>${Math.floor(overtimeMinutes1 / 60)} jam ${Math.floor(
          overtimeMinutes1 % 60
        )} menit</td>
                <td>${Math.floor(roundedOvertime2 / 60)} jam ${
          roundedOvertime2 % 60
        } menit</td>
                <td>${Math.floor(roundedOvertime3 / 60)} jam ${
          roundedOvertime3 % 60
        } menit</td>
            </tr>`;
      }
