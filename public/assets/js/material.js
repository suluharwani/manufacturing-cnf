...
$('.tambahMaterial').on('click', function() {
  $.when(
    ...
    if (Array.isArray(typesData) && Array.isArray(satuanUkuranData)) {
      ...
      // Generate kode otomatis berdasarkan type pertama yang dipilih
      const firstTypeCode = typesData[0] ? typesData[0].kode : 'DEF';
      const newMaterialId = parseInt(lastMaterialId) + 1;
      const autoGeneratedCode = `${firstTypeCode}${newMaterialId.toString().padStart(3, '0')}`;
      ...
      Swal.fire({
        ...
        <form id="form_add_data">
          <div class="form-group">
            <label for="kode">Kode</label>
            <input type="text" class="form-control" id="kode" value="${autoGeneratedCode}" placeholder="Kode">
            <small class="form-text text-muted">Kode akan digenerate otomatis berdasarkan jenis material</small>
          </div>
          <div class="form-group">
      ...
      $(document).on('change', '#type', function() {
        ...
        const newMaterialId = parseInt(lastMaterialId) + 1;
        const newAutoGeneratedCode = `${typeCode}${newMaterialId.toString().padStart(3, '0')}`;
